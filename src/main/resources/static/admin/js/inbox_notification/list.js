$(function() {
   ...
   // $("#user-list-li").parent().addClass("in");
    ...
    $.ajax({
        url: '/admin/inbox/notification/initPage',
        data: 'page=' + page,
...
function loadList(){
	...
	// 查询列表
	$.ajax({
        url : '/admin/inbox/notification/load',
        ...
        buttons: {
            primary: {
                text: 'OK', style: 'primary', action: function () {
                    $.ajax({
                        url : '/admin/inbox/notification/delete',
                        method: "POST",
                        data:'id='+id,
                        success  : function(data) {
...
$("#dataList").on('click','.user-edit',function () {
    var id = $(this).parent().data("id");
	window.location.href= "/admin/inbox/notification/edit/"+id;
});

//to add page
$('#btnNew').on('click',function(){
    window.location.href= "/admin/inbox/notification/add";
});
