$(function() {
   ...
   // $("#user-list-li").parent().addClass("in");
    ...
    $.ajax({
        url: '/admin/dataset/question/answer/initPage',
...
function loadList(){
	...
	// 查询列表
	$.ajax({
        url : '/admin/dataset/question/answer/load',
        ...
        buttons: {
            primary: {
                text: 'OK', style: 'primary', action: function () {
                    $.ajax({
                        url : '/admin/dataset/question/answer/delete/'+id,
                        method: "POST",
                        success  : function(data) {
...
$("#dataList").on('click','.user-edit',function () {
    var id = $(this).parent().data("id");
	window.location.href= "/admin/dataset/question/answer/edit/"+id;
});
//to add page
$('#addAnswer').on('click',function(){
    var questionId = $("#questionId").val();
	window.location.href= "/admin/dataset/question/answer/add/"+questionId;
});
