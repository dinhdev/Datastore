$(function() {
    ...
    $("#data-sales-list-li").attr("class","active");
...
function loadList(){
	...
	// 查询列表
	$.ajax({
        url : '/admin/data_sales/load',
        data : 'page='+page+"&name="+param,
        success  : function(data) {
        	$("#dataList").html(data);
        	var price = $('#pay_total').val();

            $('#pay_price').html("Total:"+price+"$");
		}
    });
}
        ...
        buttons: {
            primary: {
                text: 'OK', style: 'primary', action: function () {
                    $.ajax({
                        url : '/admin/data_sales/delete',
                        method: "POST",
                        data:'id='+id,
                        success  : function(data) {
                            if(data.resultCode == 'success'){
                                autoCloseAlert(data.errorInfo,1000);
                                window.location.href = "/admin/data_sales/list";
                            }else{
                                autoCloseAlert(data.errorInfo,1000);
                            }
                        }
...
$("#dataList").on('click','.user-edit',function () {
    var id = $(this).parent().data("id");
	window.location.href= "/admin/data_sales/edit/"+id;
});
