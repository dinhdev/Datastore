$(function() {
   ...
   // $("#user-list-li").parent().addClass("in");
    ...
    $.ajax({
        url: '/vendor/inbox/notification/initPage',
        data: 'page=' + page,
...
function loadList(){
	...
	// 查询列表
	$.ajax({
        url : '/vendor/inbox/notification/load',
        ...
        buttons: {
            primary: {
                text: 'OK', style: 'primary', action: function () {
                    $.ajax({
                        url : '/vendor/inbox/notification/delete',
                        method: "POST",
                        data:'id='+id,
                        success  : function(data) {
...
$("#dataList").on('click','.user-edit',function () {
    var id = $(this).parent().data("id");
	window.location.href= "/vendor/inbox/notification/edit/"+id;
});
