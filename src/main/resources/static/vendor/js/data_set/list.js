...
$("#dataList").on('click','.user-delete',function () {
    ...
    new $.flavr({
        content: 'The data sets series will be deleted. Do you Confirm? ',
        buttons: {
            primary: {
                text: 'OK', style: 'primary', action: function () {
                    $.ajax({
                        url : '/vendor/data_set/delete',
                        method: "POST",
                        data:'id='+id,
                        success  : function(data) {
                            if(data.resultCode == 'success'){
                                autoCloseAlert(data.errorInfo,1000);
                                window.location.href = "/vendor/dataset_management/list";
                            }else{
                                autoCloseAlert(data.errorInfo,1000);
                            }
                        }
...
$("#dataList").on('click','.user-edit',function () {
    ...
    var id = $(this).parent().data("id");
	window.location.href= "/vendor/dataset_management/edit/"+id;
...
// 跳转编辑页
$("#dataList").on('click','.series-add',function () {

    var id = $(this).parent().data("id");
    window.location.href= "/vendor/data_series/add/"+id;
