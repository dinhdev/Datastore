...
$(function(){
    ...
    $("#data-set-add-li").attr("class","active");

    $('#has_series > option').each(function(){
        if(0 == $("#has_series").attr('value')){
...
$('#btnDownUrlGen').on('click',function() {
    ...
    $.ajax({
        url:'/vendor/data_set/generate_s3Url',
        type:'POST',
        data:{
            bucket_name: bucket_name,
            s3_file_key: s3_file_key
...
$('#category').on('change', function () {
    ...
    if(category_value == 4){
        $('label[for=asset_class]').show();
        $('#asset_class').show();
    }else{
        $('label[for=asset_class]').hide();
        $('#asset_class').hide();
