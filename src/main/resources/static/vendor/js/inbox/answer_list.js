$(function() {
   ...
   // $("#user-list-li").parent().addClass("in");
    ...
    $.ajax({
        url: '/vendor/inbox/question/answer/initPage',
...
function loadList(){
	...
	// 查询列表
	$.ajax({
        url : '/vendor/inbox/question/answer/load',
        ...
        buttons: {
            primary: {
                text: 'OK', style: 'primary', action: function () {
                    $.ajax({
                        url : '/vendor/inbox/question/answer/delete/'+id,
                        method: "GET",
                        success  : function(data) {
                            autoCloseAlert(data.errorInfo,1000);
                            window.location.href = "/vendor/inbox/question/answer/list/"+questionId;
...
$("#dataList").on('click','.user-edit',function () {
    var id = $(this).parent().data("id");
	window.location.href= "/vendor/inbox/question/answer/edit/"+id;
});
//to add page
$('#addAnswer').on('click',function(){
    var questionId = $("#questionId").val();
	window.location.href= "/vendor/inbox/question/answer/add/"+questionId;
});
